...
- the two patterns the rules match are held in variables, and all three loops are `[[ ]]` rather than `while case … esac do`, so tree-sitter's bash grammar can parse this file. It could not before, which made every rule a comment checker has unreliable over it. Two causes, not one: the compound condition needed a `;` before `do`, and the bracket class holding an escaped `)` is rejected however it is written — inside `[[ ]]` bash refuses the escape and inside `case` it is mandatory, so only a pattern in a variable satisfies both
